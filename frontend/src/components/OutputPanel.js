...
export default function OutputPanel({imageUrl, isFetching, error}){
    let outputArray;
    if(imageUrl) outputArray = [imageUrl, 
    ...
    useEffect(()=>{
        if(outputArray) setActiveImage(outputArray[0]);
    },[imageUrl]);
    ...
    return  <div className="w-5/6 h-full my-auto relative shadow-xl space-y-5 bg-white bg-opacity-90  border-#7264a9 rounded-3xl p-5 py-10 mx-8">
        ...
        <ul className="h-1/6 flex flex-row rounded-lg p-3 pr-5 bg-violet-300 bg-opacity-20 justify-center items-center object-contain mx-auto" >
         {imageUrl && !isFetching && outputArray.map((item,index)=>{
            return <li className="relative inline-block"key={index}>
                <img className={`h-20 w-20 m-3 rounded-2xl ${activeImage===item && " border-4 border-yellow-500"}`} src={item} onClick={ (()=>setActiveImage(item))}/>
                {isSelect && <button className={`absolute top-0 right-0 transform translate-y-4 -translate-x-1 border-2 border-gray-500 text-gray-500 bg-white bg-opacity-50 rounded-full px-3 text-lg font-bold`}
                            onClick={()=>handleSelect(item)}>{selected.includes(item) ? 'v' : <br />}</button> }
            </li>
         })}
        </ul>
        {/* <DownloadBtn imageUrl={imageUrl} isFetching={isFetching}/> */}
        <button className="absolute bottom-8 right-8 disabled:text-gray-700 disabled:cursor-not-allowed" onClick={()=>{setIsSelect(true)}} disabled={isFetching || !imageUrl}>{!isSelect ? 'Select': 'Save'}</button>
    </div>
}
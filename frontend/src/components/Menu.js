...
export default function Menu(){
    ...
    const {
      ...
      } = useQuery({queryKey: ['webtoons'], queryFn: getWebtoons, options: {
        refetchOnWindowFocus:true,
        refetchOnMount: true,
        staleTime: Infinity,
        cacheTime: Infinity,
    ...
    return<div className="flex flex-col bg-gradient-to-bl from-[#0f0417] to-[#24263d] h-screen w-[220px] float-left overflow-hidden">
        ...
        <div className="flex flex-col py-4 h-2/3">
            ...
            <div className="flex flex-col h-auto max-h-1/2 overflow-y-auto overflow-x-hidden no-scrollbar">
                {isLoadingWebtoons && <img className="h-[8%] w-auto m-auto" src={generalLoading}/>}
                {webtoonsData && webtoonsData.webtoonList && webtoonsData.webtoonList.map((webtoon, index)=> <div className={`flex flex-row h-11 p-1 ${webtoonName===webtoon ? 'text-yellow-500 bg-gray-950' : 'text-white'} hover:bg-gray-950`}>

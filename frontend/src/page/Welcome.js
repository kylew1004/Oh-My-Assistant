        ...
        <div className="min-h-[260px] w-full flex flex-col">
          ...
          <p className="form-actions h-14 relative flex justify-end w-full mb-3 ">
            <Link
              ...
              className={`button flex h-14 w-1/2 absolute ${inactive}-[23px] bg-gray-300 text-gray-500 rounded-full float-left`}
            >
            ...
            <Link
              ...
              className={`button flex h-14 w-1/2 absolute ${active} text-black rounded-full float-right`}
            >
              <label className="font-bold m-auto">
...
export async function action({ request }) {
  ...
  if (mode == "login") {
    ...
    if (data.get("password") !== data.get("confirm-password"))
      return { error: "Password and Confirim-password do not match." };
